...
def findCard(imgWarpColor, matchingCard):
    ...
    cardinfo = cardData.compareCards2(hashes)

    # If a matching card was found, print its information and return True and an image of the card
    if cardinfo is not None:
        a = getFoundCardData(cardinfo)  # Displays an image containing card info
        return True, getMatchingCard(cardinfo['Card Number']), a

    # If no matching card was found, return False & black image
    return False, matchingCard, matchingCard

# Returns the matching card image given the card number - this is outdated and now just returns a dummy image
...
def CheckInventory(cardinfo):
    set = cardinfo['Card Set']
    ...
    for row in csvreader:
        if set == row[1]:
            changedNum = modify_strings(row[2])
            if number == changedNum:
                flag = True
...
def modify_strings(string):
    modified = ''
    if '/' in string:
        modified = string.split('/')[0]  # Take the part before the slash
    else:
        modified = string  # Leave it unchanged if no slash
    return modified
...
def getFoundCardData(cardinfo):
    ...
    flag, variants = CheckInventory(cardinfo)
