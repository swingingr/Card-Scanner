...
def readCard():
    ...
    matchingCard = np.zeros((utils.getHeightCard(), utils.getWidthCard(), 3), np.uint8)
    while True:
        ...
        if len(corners) == 4:
            found, null, matchingCard = utils.findCard(imgWarpColored.copy(), matchingCard)  # Check to see if a matching card was found
